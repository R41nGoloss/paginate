...
module.exports = function(options) {
	...
	function paginate(itemCount, itemsPerPage, current, query) {
		...
		var param = function toUrl(object, prefix) {
		  var result = '';
		  var key = '';
		  var postfix = '&';
		  ...
		  for (i in object) {
		    if (!prefix) {
		      key = querystring.escape(i);
		    } else {
		      key = prefix + '[' + querystring.escape(i) + ']';
		    }

		    if (typeof(object[i]) == 'string') {
		      result += key + '=' + querystring.escape(object[i]) + postfix;
		      continue;
		    }

		    if (typeof(object[i]) == 'object' || typeof(object[i]) == 'array') {
		      result += param(object[i], key) + postfix;
		      continue;
		    }

		    if (object[i].toString) {
		      result += key + '=' + querystring.escape(object[i].toString()) + postfix;
		      continue;
		    }
		  }
